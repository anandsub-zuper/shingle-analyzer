...
const RoofAnalyzer = () => {
  ...
  useEffect(() => {
    ...
    const savedLocation = localStorage.getItem('roofanalyzer_location');
    if (savedLocation) {
      try {
        ...
        if (locationAge < 24 * 60 * 60 * 1000) {
          ...
          console.log('Loaded saved location from localStorage');
        }
      } catch (e) {
  ...
  const handleLocationDetected = (data) => {
    ...
    setLocationData(locationWithTimestamp);
    console.log('Location detected:', locationWithTimestamp);
    
    // Save to localStorage for future use
    try {
      localStorage.setItem('roofanalyzer_location', JSON.stringify(locationWithTimestamp));
    } catch (e) {
    ...
    try {
      ...
      if (locationData) {
        requestData.location = {
          ...
          address: locationData.address?.fullAddress
        };
        
        // Add property data if available
        if (locationData.propertyData) {
          requestData.propertyData = {
            type: locationData.propertyData.propertyType,
            yearBuilt: locationData.propertyData.yearBuilt,
            bedrooms: locationData.propertyData.bedrooms,
            bathrooms: locationData.propertyData.bathrooms,
            squareFootage: locationData.propertyData.squareFootage
          };
        }
      }
      ...
      const response = await callApiWithRetry(requestData);
  ...
  const callApiWithRetry = async (requestData, attempt = 0) => {
    ...
    try {
      ...
      if (shouldRetry && isMounted.current) {
        ...
        if (isMounted.current) {
          // Recursively retry
          return callApiWithRetry(requestData, attempt + 1);
        } else {
  ...
  return (
    <div className="shingle-analyzer-container">
      ...
      {/* Add the HomeLocationDetector component */}
      <HomeLocationDetector onLocationDetected={handleLocationDetected} />
      ...
      <div className="upload-section">
        <div className="form-group">
          ...
          <p className="input-help-text">
            ...
            {locationData && (
              <span className="location-enabled-message"> • Location data will be used for better analysis</span>
            )}
          </p>
        </div>
      ...
      {debugMode && (results || error) && (
        <DebugHelper apiResponse={results} error={error} locationData={locationData} />
      )}
        ...
        <div className="results-container">
          ...
          <div className="info-section">
            ...
            <p className="info-text">
              ...
              {locationData && (
                <span> Location data is used to provide region-specific analysis and recommendations.</span>
              )}
            </p>
          </div>
