...
// server.js with location and property data integration
...
app.post('/api/analyze-shingle', async (req, res) => {
  ...
  try {
    ...
    const { image, location, propertyData } = req.body;
    ...
    // Enhanced prompt for OpenAI Vision API with location awareness
...
`;
    ...
    try {
        ...
        if (typeof responseContent === 'string') {
          ...
          if (jsonMatch) {
            try {
              ...
              console.log("Successfully parsed JSON from response content");
            } catch (jsonParseError) {
              console.warn("Could not parse extracted JSON match:", jsonParseError);
            }
          }
        }
      } catch (jsonError) {
      ...
      // Add location data to the response if available
      if (location) {
        processedData.locationData = {
          coordinates: location.coordinates,
          address: location.address
        };
        console.log("Added location data to the response");
      }
